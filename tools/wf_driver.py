...
def build_windows(start, end, train_m, test_m):
    """Return list of (train_start_YYYY-MM-DD, test_end_YYYY-MM-DD)."""
    ...
    out=[]
    while True:
        te = add_months(ts, test_m) - timedelta(seconds=1)  # end of test period
        if te > e: break
        tr0 = add_months(ts, -train_m)                      # TRAIN START (not test start!)
...
def shard_done(run_tag, wi, i):
    ...
    if not t.exists(): return False
    try: return (sum(1 for _ in open(t)) - 1) > 0
    except Exception: return False

def window_complete(run_tag, wi, shards):
    return all(shard_done(run_tag, wi, i) for i in range(shards))
def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--symbols", required=True, help="space- or comma-separated")
    ap.add_argument("--start", required=True)  # overall
    ap.add_argument("--end", required=True)    # overall
    ...
    if not args.symbols.strip():
        raise SystemExit("Error: --symbols is empty")

    wins = build_windows(args.start, args.end, args.train_months, args.test_months)
    if args.win_from is not None or args.win_to is not None:
    ...
    for off,(tr_start, te_end) in enumerate(wins):
        ...
        if window_complete(args.run_tag, wi, args.shards):
            print(f"== Window {wi:02d} {tr_start}->{te_end}: complete, skip")
            continue
        print(f"== Window {wi:02d} {tr_start}->{te_end}: launching shards")
        procs=[]
        for i in range(args.shards):
            if shard_done(args.run_tag, wi, i):
                print(f"   - shard {i}: ok, skip"); continue
            cmd=[py,"run_backtest.py","--source","local",
                 ...
                 "--shard-index", str(i), "--shard-count", str(args.shards),
                 "--seed", str(args.seed)]
